...
echo "<script>window.BNT_CSRF_TOKEN=" . json_encode(bnt_csrf_token()) . ";document.querySelectorAll(\"form\").forEach(function(form){var method=(form.getAttribute(\"method\")||\"get\").toLowerCase();if(method!==\"post\"){return;}if(form.querySelector(\"input[name='csrf_token']\")){return;}var input=document.createElement(\"input\");input.type=\"hidden\";input.name=\"csrf_token\";input.value=window.BNT_CSRF_TOKEN;form.appendChild(input);});</script>\n";
...
db_op_result ($db, $res, __LINE__, __FILE__);
$result = $res->fields;
$mySEC = ($sched_ticks * 60) - (TIME () - (int) ($result['last_run'] ?? 0));
if ($mySEC < 0)
{
    $mySEC = 0;
}
...
{
    define('BNT_FOOTER_RENDERED', true);
}
?>
<?php ob_end_flush();?>
