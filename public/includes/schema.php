...
$db->execute("CREATE TABLE IF NOT EXISTS {$db->prefix}zones (" .
             ...
             "is_void enum('Y','N') DEFAULT 'N' NOT NULL," .
             "PRIMARY KEY(zone_id)," .
             "KEY zone_id(zone_id)" .
             ")");
$err = TRUEFALSE(0, $db->ErrorMsg(),"No errors found", $db->ErrorNo() . ": " . $db->ErrorMsg());
...
$db->Execute("CREATE TABLE IF NOT EXISTS {$db->prefix}ngai (" .
             "ngai_id char(40) NOT NULL," .
             "ship_type enum('scout','fighter','spy','cruiser','battleship','freighter') DEFAULT 'fighter' NOT NULL," .
             "faction tinyint unsigned DEFAULT '1' NOT NULL," .
             "home_sector int unsigned DEFAULT '0' NOT NULL," .
             "hostile_on_scan enum('Y','N') DEFAULT 'N' NOT NULL," .
             "PRIMARY KEY (ngai_id)," .
             "KEY ship_type (ship_type)," .
             "KEY faction (faction)" .
             ")");
$err = TRUEFALSE(0, $db->ErrorMsg(),"No errors found", $db->ErrorNo() . ": " . $db->ErrorMsg());
Table_Row("Creating ngai Table","Failed","Passed");

$db->Execute("CREATE TABLE IF NOT EXISTS {$db->prefix}ngai_hostile (" .
             "hostile_id int unsigned NOT NULL auto_increment," .
             "ngai_id char(40) NOT NULL," .
             "target_ship_id int DEFAULT '0' NOT NULL," .
             "triggered_at BIGINT(20) NOT NULL," .
             "PRIMARY KEY (hostile_id)," .
             "KEY ngai_id (ngai_id)" .
             ")");
$err = TRUEFALSE(0, $db->ErrorMsg(),"No errors found", $db->ErrorNo() . ": " . $db->ErrorMsg());
Table_Row("Creating ngai_hostile Table","Failed","Passed");
...
$db->Execute("CREATE TABLE IF NOT EXISTS {$db->prefix}scheduler (" .
             ...
             "last_run BIGINT(20)," .
             "next_run BIGINT(20) DEFAULT NULL," .
...
$db->Execute("CREATE TABLE IF NOT EXISTS {$db->prefix}password_resets (" .
...
Table_Row("Creating password_resets Table","Failed","Passed");
