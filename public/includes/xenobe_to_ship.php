...
function xenobetoship($ship_id)
{
  ...
  // VERIFY NOT ATTACKING A FRIENDLY NGAI SHIP
  // Inter-faction NGAI combat is allowed only when $ngai_interfaction_attack is set
  global $ngai_interfaction_attack;
  if (strstr($targetinfo[email], '@ngai') && empty($ngai_interfaction_attack))
  {
    $resb = $db->Execute("UNLOCK TABLES");
    db_op_result ($db, $resb, __LINE__, __FILE__, $db_logging);
    return;
