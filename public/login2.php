...
if ($playerfound)
{
    if (!$banned && bnt_password_verify_legacy($pass, $playerinfo['password']))
    {
        if (bnt_password_needs_upgrade($playerinfo['password']))
        {
            $upgradedPassword = bnt_password_hash_value($pass);
            $db->Execute("UPDATE {$db->prefix}ships SET password=? WHERE ship_id=?", array($upgradedPassword, $playerinfo['ship_id']));
        }
    ...
    else
    {
        ...
        echo $l_login_4gotpw1a . "<br><br>Please try again. Attempt logged with IP address of " . $ip . "...";
