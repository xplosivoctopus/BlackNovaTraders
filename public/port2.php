...
#           }
    ...
    elseif ($sectorinfo['port_type'] != "none")
    {
        ...
        $using_modal_trade_inputs = false;
        $modal_trade_fields = array('ore', 'organics', 'goods', 'energy');
        foreach ($modal_trade_fields as $commodity_name)
        {
            if (isset($_POST["buy_trade_" . $commodity_name]) || isset($_POST["sell_trade_" . $commodity_name]))
            {
                $using_modal_trade_inputs = true;
                break;
            }
        }
        ...
        if ($using_modal_trade_inputs)
        {
            $ore_price = ($sectorinfo['port_type'] == "ore")
                ? $ore_price - $ore_delta * $sectorinfo['port_ore'] / $ore_limit * $inventory_factor
                : $ore_price + $ore_delta * $sectorinfo['port_ore'] / $ore_limit * $inventory_factor;
            $organics_price = ($sectorinfo['port_type'] == "organics")
                ? $organics_price - $organics_delta * $sectorinfo['port_organics'] / $organics_limit * $inventory_factor
                : $organics_price + $organics_delta * $sectorinfo['port_organics'] / $organics_limit * $inventory_factor;
            $goods_price = ($sectorinfo['port_type'] == "goods")
                ? $goods_price - $goods_delta * $sectorinfo['port_goods'] / $goods_limit * $inventory_factor
                : $goods_price + $goods_delta * $sectorinfo['port_goods'] / $goods_limit * $inventory_factor;
            $energy_price = ($sectorinfo['port_type'] == "energy")
                ? $energy_price - $energy_delta * $sectorinfo['port_energy'] / $energy_limit * $inventory_factor
                : $energy_price + $energy_delta * $sectorinfo['port_energy'] / $energy_limit * $inventory_factor;

            $trade_ore = round(abs((int) ($_POST['buy_trade_ore'] ?? 0))) - round(abs((int) ($_POST['sell_trade_ore'] ?? 0)));
            $trade_organics = round(abs((int) ($_POST['buy_trade_organics'] ?? 0))) - round(abs((int) ($_POST['sell_trade_organics'] ?? 0)));
            $trade_goods = round(abs((int) ($_POST['buy_trade_goods'] ?? 0))) - round(abs((int) ($_POST['sell_trade_goods'] ?? 0)));
            $trade_energy = round(abs((int) ($_POST['buy_trade_energy'] ?? 0))) - round(abs((int) ($_POST['sell_trade_energy'] ?? 0)));

            $cargo_exchanged = max($trade_ore, 0) + max($trade_organics, 0) + max($trade_goods, 0);
        }
        else
        {
        ...
        elseif ((!$using_modal_trade_inputs && abs ($trade_organics) > $sectorinfo['port_organics']) || ($using_modal_trade_inputs && $trade_organics > $sectorinfo['port_organics']))
        {
            echo $l_exceed_organics;
        }
        elseif ((!$using_modal_trade_inputs && abs ($trade_ore) > $sectorinfo['port_ore']) || ($using_modal_trade_inputs && $trade_ore > $sectorinfo['port_ore']))
        {
            echo $l_exceed_ore;
        }
        elseif ((!$using_modal_trade_inputs && abs ($trade_goods) > $sectorinfo['port_goods']) || ($using_modal_trade_inputs && $trade_goods > $sectorinfo['port_goods']))
        {
            echo $l_exceed_goods;
        }
        elseif ((!$using_modal_trade_inputs && abs ($trade_energy) > $sectorinfo['port_energy']) || ($using_modal_trade_inputs && $trade_energy > $sectorinfo['port_energy']))
        {
            echo $l_exceed_energy;
        }
        elseif ($using_modal_trade_inputs && ($playerinfo['ship_ore'] + $trade_ore < 0 || $playerinfo['ship_organics'] + $trade_organics < 0 || $playerinfo['ship_goods'] + $trade_goods < 0 || $playerinfo['ship_energy'] + $trade_energy < 0))
        {
            echo $l_notenough_cargo . " <br><br>";
        }
        else
