...
if (isset($destination))
{
    ...
    // Federation bounty travel penalty: +1 turn per 100,000 credits of bounty
    if ($triptime > 0)
    {
        $bounty_res = $db->Execute("SELECT SUM(amount) as total_bounty FROM {$db->prefix}bounty WHERE placed_by = 0 AND bounty_on = " . (int)$playerinfo['ship_id']);
        if ($bounty_res && !$bounty_res->EOF && $bounty_res->fields['total_bounty'] > 0)
        {
            $triptime += (int)floor($bounty_res->fields['total_bounty'] / 100000);
        }
    }
}
