...
else
{
    ...
    else
    {
        ...
        else
        {
            ...
            // NGAI Spy retaliation: queue hostile retaliation if scanned ship has hostile_on_scan='Y'
            if (strpos($targetinfo['email'], '@ngai') !== false)
            {
                $ngai_hr = $db->Execute(
                    "SELECT hostile_on_scan FROM {$db->prefix}ngai WHERE ngai_id=?",
                    array($targetinfo['email'])
                );
                if ($ngai_hr && !$ngai_hr->EOF && $ngai_hr->fields['hostile_on_scan'] === 'Y')
                {
                    $db->Execute(
                        "INSERT INTO {$db->prefix}ngai_hostile (ngai_id, target_ship_id, triggered_at) VALUES (?,?,?)",
                        array($targetinfo['email'], (int)$playerinfo['ship_id'], time())
                    );
                    playerlog($db, $playerinfo['ship_id'], LOG_RAW,
                              "You scanned an unidentified vessel — it has marked you as a hostile target.");
                }
            }
        }
