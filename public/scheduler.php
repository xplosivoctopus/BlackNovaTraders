...
#echo "<pre>[REQUEST]\n". print_r($_REQUEST, true) ."</pre>\n";
    ...
    if ($missed_res && !$missed_res->EOF)
    {
        while (!$missed_res->EOF)
        {
...
$lastrunList[$event['sched_file']] = $event['last_run'];
            ...
            if ($event['repeate'] == 'N')
            {
                ...
                {
                    $next_run_time = time() + $ticks_left * $sched_ticks * 60;
                    $resy = $db->Execute("UPDATE {$db->prefix}scheduler SET ticks_left=$ticks_left, spawn=spawn-$multiplier, next_run=$next_run_time WHERE sched_id=$event[sched_id]");
            ...
            {
                $next_run_time = time() + $ticks_left * $sched_ticks * 60;
                $resz = $db->Execute("UPDATE {$db->prefix}scheduler SET ticks_left=$ticks_left, next_run=$next_run_time WHERE sched_id=$event[sched_id]");
