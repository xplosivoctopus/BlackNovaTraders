...
class SETUPINFO_CLASS
{
    ...
    // Constructor
    function __construct($in_Value = 0)
    {
    ...
    function testdb_connection()
    {
        ...
        {
            $this->db_status['status'] = ((($db instanceof ADOConnection) && $db->IsConnected()) ? "Connected OK" : "Not Connected");
...
/*                $db_info['status'] = "Setup Info has found ".count($dbtables)." tables in the Tables List.";
    ...
    {
        $switch_info = array();
        foreach ($this->switches as $switch_name => $switch_array)
        {
